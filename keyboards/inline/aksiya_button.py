...
from utils.db_api.database import get_aksiya_items, get_aksiya_categories, get_category_by_id
...
async def categories_keyboard(lang):
    ...
    categories = await get_aksiya_categories()
...
async def items_keyboard(category, lang):
    ...
    items = await get_aksiya_items(category)
    cat = await get_category_by_id(category)
    for item in items:
        if lang == "ru":
...
async def item_keyboard(category, item_id, selected, lang):
    ...
    markup.row(
        InlineKeyboardButton(text=f"0", callback_data=select.new(key="num_choose", data="0", choosen_data=selected,
                                                                 item_id=item_id)),
        InlineKeyboardButton(text=_(f"↩️Back", locale=lang),
