...
from utils.db_api.database import get_item, get_user, get_lang, get_aksiya_item
...
@dp.callback_query_handler(text="no_call", state='aksiya')
async def no_call_fun(call: types.CallbackQuery):
    await call.answer()
...
async def get_choosen_num(call: types.CallbackQuery, callback_data: dict):
    ...
    if int(selected) != 0:
        ...
        markup = await item_keyboard(category=category, item_id=item_id, selected=selected,
...
@dp.callback_query_handler(select.filter(key="num_choose"), state='aksiya')
async def get_choosen_num(call: types.CallbackQuery, callback_data: dict):
    await call.answer()
    num = callback_data.get("data")
    selected = callback_data.get("choosen_data")
...
@dp.callback_query_handler(select.filter(key="delete_choosen"), state='*')
async def delete_choosen_num(call: types.CallbackQuery, callback_data):
    ...
    if int(selected) != 0:
        ...
        if len(selected) != 1:
            ...
            if selected == "":
                selected = 0
            markup = await item_keyboard(category=item.category_name, item_id=item_id, selected=selected,
                                         lang=await get_lang(user_id))
            await call.message.edit_reply_markup(reply_markup=markup)
        else:
            markup = await item_keyboard(category=item.category_name, item_id=item_id, selected="0",
                                         lang=await get_lang(user_id))
            await call.message.edit_reply_markup(reply_markup=markup)
    else:
        selected = 0
...
@dp.callback_query_handler(select.filter(key="delete_choosen"), state='aksiya')
async def delete_choosen_num(call: types.CallbackQuery, callback_data):
    selected = callback_data.get("choosen_data")
    item_id = int(callback_data.get("item_id"))
    item = await get_item(item_id)
...
@dp.callback_query_handler(cart_callback.filter(), state='aksiya')
async def go_cart_fun(call: types.CallbackQuery, callback_data: dict):
    item_id = callback_data.get("item_id")
    user_id = call.from_user.id
    choosen_data = callback_data.get("choosen_data")
    if int(choosen_data) != 0:
        ...
        cart.user = user
        cart.productA = item
        if int(choosen_data) >= int(item.begin_aksiya):
            count = int(item.count_a) - int(item.begin_aksiya)
            cart.amount = int(choosen_data) + int(count)
        else:
            cart.amount = choosen_data
        cart.total = int(choosen_data) * item.price
        cart.is_success = False
...
#     print(item_id)
#     markup = await about_product_btn(item_id=item_id, selected=selected, lang=user.lang, max_pages=max_pages_photo,
#                                      key=item_id,
#                                      page=current_page)
#
...
@dp.callback_query_handler(about_callback.filter(key="back_product"), state='aksiya')
async def back_product_fun(call: types.CallbackQuery, callback_data: dict):
    selected = callback_data.get("selected")
    item_id = callback_data.get("item_id")
    item = await get_item(item_id)
